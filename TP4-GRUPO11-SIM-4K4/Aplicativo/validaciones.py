...
def validar_entradas(entry_prob_compra, entry_prob_repara, entry_prob_retira,
    ...
    try:
        ...
        valor_b = validar_float(entry_b, "Valor B")
        ...
        # Validar a y b
        if valor_a < 0:
            raise ValueError("'Valor A' no puede ser menor a 0.")
        if valor_a >= valor_b:
            raise ValueError("'Valor A' debe ser menor que 'Valor B'.")
        if valor_b > tiempo_simular:
            raise ValueError("'Valor B' no puede ser mayor que el tiempo a simular.")
